...
designed; this documents it as a footgun rather than proposing a change) ·
**Status:** open, doc-only · **Found:** 2026-09-22, live "Phase 1 done-test"
verification in Resource Explorer (trellis repo) of the `data_class_match`
Data Class bootstrap.

**What:** `get_guid_for_name(name)` (and the equivalent method on every OMVS
client that shares this convention — confirmed on `DataDesigner` and
`ReferenceDataManager`) returns the literal string `"No elements found"` on
a miss, not `None`, not `""`, and not an exception. That string is truthy
in Python, so any caller that checks the result with a bare
`if not guid`/`guid or default` reads a miss as "found" and skips the
create it should have run.

**Confirmed live impact:** Resource Explorer's
`resource_explorer/surveyors/database/bootstrap_data_classes.py` used
exactly that check (`if not data_class_guid: create(...)`) for all three of
its lookups (`DataClass`, `ValidValuesSet`, `ValidValueDefinition`
keywords). Running it against a platform holding zero Data Classes reported
`Created Data Classes: 0, Skipped: 6` — every one of the six standard
classes was silently treated as "already exists" and never created. A
second, independently-introduced instance of the identical bug was found
the same day in the same directory:
`egeria_reference_catalog.py`'s `_find_existing()` used
`str(client.get_guid_for_name(qualified_name) or "")`, which the sentinel
string also defeats (`or ""` never fires on a truthy string).

**This is not a new pyegeria defect** — it's the same sentinel-return
convention already covered generally in this file's Quick Reference table
and in several fixed entries (e.g. ISSUE-42's `get_linked_projects`), but
this is the first entry pinning the specific "existence-check" failure mode
by name, since `if not guid` / `guid or default` is an easy, natural-looking
way to write that check and it silently does the wrong thing.

**Caller-side fix applied (not a pyegeria change)**: both RE call sites now
route the result through `survey_definition_reader._as_guid()`, which
rejects the sentinel because it contains whitespace (`any(ch.isspace() ...)`
), unlike a real GUID. `_as_guid` already existed and was already used
correctly elsewhere in RE (`bootstrap.py`'s canary check,
`survey_definition_reader.py` itself); these two call sites had each grown
their own, incorrect, ad hoc check instead of reusing it. See trellis
`re/data-class-seed-and-wiring` branch (bootstrap_data_classes.py,
egeria_reference_catalog.py, tests/test_bootstrap_data_classes.py,
tests/test_postgres_column_profile.py's
`test_a_miss_reading_as_pyegerias_sentinel_string_still_creates`).

**Candidate pyegeria-side improvement, NOT requested/applied here** — per
this repo's standing convention, logging only: `get_guid_for_name` (and
siblings sharing the convention) could raise a dedicated
`ElementNotFoundError`/return `None` instead of the sentinel string, which
would make `if not guid` correct instead of a trap. Not proposing this
without the owner's approval, since existing callers across pyegeria and
its consumers may already depend on the string return value.
...
on the same widely-shared `Referenceable` bundle as `Security Tags` above,
