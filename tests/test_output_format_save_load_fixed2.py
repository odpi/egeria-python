...
# Import these separately to avoid global variable issues
import pyegeria._output_formats as output_formats
...
def test_save_load_all_format_sets():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(data) == len(output_formats.output_format_sets), f"Expected {len(output_formats.output_format_sets)} format sets, got {len(data)}"
        
        # Save the original format sets and create a new empty FormatSetDict
        original_format_sets = output_formats.output_format_sets.copy()
        output_formats.output_format_sets = FormatSetDict()
        ...
        assert len(output_formats.output_format_sets) == len(original_format_sets), f"Expected {len(original_format_sets)} format sets, got {len(output_formats.output_format_sets)}"
        
        # Restore the original format sets
        output_formats.output_format_sets = original_format_sets
...
def test_save_load_subset_format_sets():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Save the original format sets and set the global variable to the test format sets
        original_format_sets = output_formats.output_format_sets.copy()
        output_formats.output_format_sets = test_format_sets
        
        # Load the format sets from the file
        load_output_format_sets(file_path, merge=True)
        
        # Check that the loaded format sets match the subset
        assert len(output_formats.output_format_sets) == len(subset_names), f"Expected {len(subset_names)} format sets, got {len(output_formats.output_format_sets)}"
        
        # Check that the loaded format sets have the correct names
        for name in subset_names:
            assert name in output_formats.output_format_sets, f"Format set {name} not found in loaded format sets"
...
def test_merge_format_sets():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "ExistingFormatSet" in output_formats.output_format_sets, "Existing format set not found in merged format sets"
        assert "CustomFormatSet" in output_formats.output_format_sets, "Custom format set not found in merged format sets"
        
        # Check that the merged format sets have the correct number of format sets
        assert len(output_formats.output_format_sets) == 2, f"Expected 2 format sets, got {len(output_formats.output_format_sets)}"
...
def test_user_format_sets_directory():
    ...
    original_dir = output_formats.USER_FORMAT_SETS_DIR
    
    try:
        # Create a temporary directory for the test
        with tempfile.TemporaryDirectory() as temp_dir:
            # Set the USER_FORMAT_SETS_DIR to the temporary directory
            output_formats.USER_FORMAT_SETS_DIR = temp_dir
            ...
            os.makedirs(output_formats.USER_FORMAT_SETS_DIR, exist_ok=True)
            file_path = os.path.join(output_formats.USER_FORMAT_SETS_DIR, "user_custom_format_sets.json")
            user_format_sets.save_to_json(file_path)
            ...
            output_formats.load_user_format_sets()
            
            # Check that the user format sets were loaded
            assert "UserCustomFormatSet" in output_formats.output_format_sets, "User custom format set not found in loaded format sets"
            
            # Restore the original format sets
            output_formats.output_format_sets = original_format_sets
    finally:
        # Restore the original USER_FORMAT_SETS_DIR
        output_formats.USER_FORMAT_SETS_DIR = original_dir
