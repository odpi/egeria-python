...
This module is for testing the metadata expert view service class and methods.
...
class MetadataClient(MetadataExpert):
    """Compatibility wrapper so existing harness calls keep working after consolidation."""

    def __init__(self, view_server, platform_url, user_id="erinoverview", user_pwd="secret", token=None):
        super().__init__(view_server, platform_url, user_id, user_pwd, token)


# Backward-compatible alias used by existing harness calls in this file.
MetadataExplorer = MetadataClient
...
class TestMetadataExpertHarness:
