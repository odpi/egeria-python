...
class TestPlatform:
    ...
    def test_activate_server_supplied_config(self, server: str = good_server_2):
        server = "test-store"
        config_body = {
    ...
    def test_activate_server_if_down(self, server:str = good_server_4):
        try:
            p_client = Platform(server, self.good_platform1_url, self.good_user_1)
            response = p_client.activate_server_if_down(server)
            print(f"\n\n\t  activation success was {response}")
            if response is False:
                print("Check that the server is configured..")
            assert True
    ...
    def test_activate_servers_on_platform(self):
        try:
            ...
            if response is False:
                print("Check that all servers are configured")
            server_list = p_client.get_known_servers()
            print(f"\n\n\tKnown servers on the platform are: {server_list}")
            ...
            print_exception_response(e)
            assert e.related_http_code is "200", "Invalid parameters or timeout"

    def test_activate_platform(self, server:str = good_server_1):
        try:
            p_client = Platform(server, self.good_platform1_url, self.good_user_1)
            p_client.activate_platform("laz",
                                       ["active-metadata-store","simple-metadata-store","view-server"])
