...
def _term_command_level(term: dict) -> str:
    """
    Read a term's commandLevel (Basic/Advanced), tagged in Additional
    Properties by generate_dr_help.py. Missing/pre-migration terms default
    to "Basic" (inclusive default, matching create_help_terms' own
    commands[command].get("level", "Basic") default).
    """
    props = term.get("properties") or {}
    additional = props.get("additionalProperties") or {}
    return additional.get("commandLevel", "Basic")


def _filter_usage_table_by_level(usage_md: str, level: str) -> str:
    """
    Filter a Usage markdown table's rows by their own 'Level' column,
    generated by generate_dr_help.py's create_help_terms. level="all" (or
    "advanced") returns the table unfiltered; level="basic" (the default)
    keeps only rows tagged Level=Basic, matching the semantics
    generate_dr_help.py itself used to apply at generation time before
    generation was collapsed into one combined pass.

    Tables without a Level column (pre-migration content) are returned
    unfiltered -- nothing to filter on.
    """
    if level.lower() in ("all", "advanced") or not usage_md or not usage_md.strip():
        return usage_md

    lines = usage_md.split("\n")
    ...
    out = lines[:header_idx + 2]  # header + separator row, kept as-is
    for line in lines[header_idx + 2:]:
        if not line.strip().startswith("|"):
            out.append(line)
            continue
        cells = [c.strip() for c in line.strip().strip("|").split("|")]
        if len(cells) > level_col and cells[level_col] != "Basic":
            continue
        out.append(line)
    return "\n".join(out)
...
def display_command_terms(
    ...
    level : str, optional
        "basic" (default) shows only Basic-level commands, and within each
        shown command's Usage table only Basic-level attribute rows. "all"
        (or "advanced") shows every command and every attribute row
        unfiltered. Both are tagged per-term/per-row by generate_dr_help.py.
    ...
    def generate_table(search_string: str, glossary_guid: str) -> Table:
        ...
        if isinstance(terms, str):
            ...
            sys.exit(0)
        if level.lower() not in ("all", "advanced"):
            terms = [t for t in terms if _term_command_level(t) == "Basic"]
        sorted_terms = sorted(
        ...
        for term in sorted_terms:
            ...
            usage = _filter_usage_table_by_level(props.get("usage", "---"), level)
    ...
    if mode in ("md", "md-html"):
        ...
        if mode == "md":
            ...
            for term in sorted_terms:
                ...
                usage = _filter_usage_table_by_level(props.get("usage", "") or "", level)
                lines.append(f"## {name}")
                lines.append("")
            ...
            for term in sorted_terms:
                ...
                name = props.get("displayName", "---") or "---"
                desc = props.get("description", "") or ""
                usage_md = _filter_usage_table_by_level(props.get("usage", "") or "", level)
...
def main():
    ...
    parser.add_argument("--level", choices=["basic", "all", "advanced"], default="basic",
                         help="basic (default): only Basic-level commands, and only Basic-level attribute "
                              "rows within them. all/advanced: every command and every attribute row.")
