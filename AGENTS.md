...
- Edit compact command JSON (attributes/bundles/commands): `dr_egeria_spec_editor` (one-time `uv sync --extra spec-editor`) — local FastAPI+JS editor at `http://localhost:8420`, writes directly to `md_processing/data/compact_commands/*.json`, review via `git diff`. See `md_processing/data/compact_commands/README.md`. **This is Claude's edit path too, not just a human UI** (confirmed 2026-08-05): make compact-spec changes through this app's REST API (`GET/POST/PUT/DELETE /api/families/{family}/attributes|bundles|commands/{name}`, `POST /api/validate/{family}`) rather than hand-editing the JSON directly. **Tinderbox is retired** (confirmed by the user 2026-08-05, same day) — this repo's compact JSON is the sole system of record now, no mirroring/re-export step needed after an edit.
