...
def load_app_config(env_file: str | None = None):
    return import_module('pyegeria.core.config').load_app_config(env_file)

def get_app_config(env_file: str | None = None):
    return import_module('pyegeria.core.config').get_app_config(env_file)

class PyegeriaSettings:
    def __new__(cls, *args, **kwargs):
        # Construct a real settings instance from pyegeria.core.config
        return import_module('pyegeria.core.config').PyegeriaSettings(*args, **kwargs)

    @classmethod
    def with_env_file(cls, env_file: str):
        return import_module('pyegeria.core.config').PyegeriaSettings.with_env_file(env_file)
