...
from pyegeria._globals import NO_GUID_RETURNED, max_paging_size
...
class AssetMaker(ServerClient):
    ...
    @dynamic_catch
    async def _async_create_asset(self, body: dict | NewElementRequestBody | None = None) -> str:
        ...
        """
        url = f"{self.asset_command_root}/assets"
        return await self._async_create_element_body_request(url, ["AssetProperties"], body)

    @dynamic_catch
    def create_asset(self, body: dict | NewElementRequestBody | None = None) -> str:
        """Create an asset.
        ...
        return loop.run_until_complete(self._async_create_asset(body))
    ...
    async def _async_create_asset_from_template(
        ...
        """
        url = f"{self.asset_command_root}/assets/from-template"
    ...
    async def _async_update_asset(
        ...
        """
        url = f"{self.asset_command_root}/assets/{asset_guid}/update"
    ...
    async def _async_delete_asset(
        ...
        PyegeriaException
            ...
            Egeria errors.
        """
        url = f"{self.asset_command_root}/assets/{asset_guid}/delete"
    ...
    async def _async_find_assets(
        ...
        if body is None:
            ...
            body["startsWith"] = starts_with
            body["endsWith"] = ends_with
            body["ignoreCase"] = ignore_case

        body_dict = body if isinstance(body, dict) else body.model_dump(mode="json", by_alias=True, exclude_none=True)
        body_dict["startFrom"] = start_from
        body_dict["pageSize"] = page_size

        url = f"{self.asset_command_root}/assets/by-search-string"
        response = await self._async_make_request("POST", url, body_dict)
        return response.json().get("elements", [])
