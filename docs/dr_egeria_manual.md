...
- **Solution Architect**: Manage solution blueprints, components, information supply chains, and design patterns (e.g., `Create Design Pattern`, `Link Nested Design Patterns`, `Link Specialized Design Patterns`, `Link Related Design Patterns`).
  ...
  - **Known gap**: `Link Concept Design` needs an existing `ConceptModel` container element, but there's no dedicated Dr.Egeria create command for `ConceptModel` yet — no REST endpoint for it is documented in the `.http` ground truth either. Create one via pyegeria/the Egeria REST API directly (or with a template GUID, if a suitable template exists) before using this command; it stays effectively untestable via markdown alone until that lands.
