...
- **Curation**: Apply classifications and relationships to *existing* Referenceable elements after the fact — this family creates no elements of its own, it curates ones created elsewhere. Every command names a `Target Element` (Reference Name) and either a classification level/status or a second element to relate to.
  ...
  - **Not implemented** (`Classify`/`Declassify` Policy Management Point): this is a real Egeria classification type, but no pyegeria SDK method exists for it yet (confirmed via `ClassificationExplorer` — not even a stub, and no matching Egeria PR has shipped one). It parses and validates but isn't registered with the dispatcher, so `--process` reports "No processor registered," same as an unimplemented command in any other family. Add the pyegeria method first, then register it in `register_curation_processors()` (`md_processing/dr_egeria.py`) by adding its `OM_TYPE` to `CLASSIFICATION_METHODS` in `md_processing/v2/curation.py`.
