...
- **Asset Maker** (added 2026-09-11): Instantiate Open Metadata catalog templates via `pyegeria/omvs/automated_curation.py`'s create-from-template surface — no update path, since the underlying OMVS methods are create-only. `Create Element` is the generic, Advanced-only mechanism (raw `typeName`/`templateGUID`/`placeholderPropertyValues`); twelve Basic-level, type-specific wrappers cover the common cases: `Create Secrets Store Element`, `Create Kafka Server Element`, `Create CSV Data File Element`, `Create Postgres Server Element`, `Create Postgres Database Element`, `Create Folder Element`, `Create UC Server Element`, `Create UC Catalog Element`, `Create UC Schema Element`, `Create UC Table Element`, `Create UC Function Element`, `Create UC Volume Element`. Live-verified: `Create Schema Type` (see below) and `Create CSV Data File Element` round-tripped against a running server; `Create Kafka Server Element` currently fails on a server with no "Apache Kafka Server" catalog template registered — a pre-existing gap in that server's template catalog, not a defect in this command.
