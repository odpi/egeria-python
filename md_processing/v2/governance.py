...
class GovernanceLinkProcessor(AsyncBaseCommandProcessor):
    ...
    def supports_target_element_lookup(self) -> bool:
        # A relationship-only processor -- there is no target Referenceable
        # element for AsyncBaseCommandProcessor.execute() to look up by
        # qualified name. Without this override, step 5's Create<->Update
        # upsert-transition logic (as_is_element always None here + no
        # qualified_name to plan against) silently rewrites every "Update
        # Certification"/"Update License" command to "Create" instead --
        # confirmed live (ISSUE-68 follow-up) and reproduced independently
        # against the pre-existing "Update Lineage Relationship" command
        # too, so this isn't specific to the new commands added here.
        return False
    ...
    async def apply_changes(self) -> str:
        ...
        if verb == "Update" and object_type in {"Certification", "License"}:
            # Certification/License are MULTI_LINK (see
            # pyegeria.core.relationship_multiplicity) -- more than one may
            # exist between the same pair, so updating one requires its own
            # relationship GUID, not the pair of element GUIDs the rest of
            # this method resolves via endpoint_map below. Handled here,
            # before that resolution, since "Update Certification"/"Update
            # License" don't carry the "Certification Type"/"Referenceable"
            # attributes endpoint_map would otherwise require. Unlike every
            # other object_type here, Update isn't auto-generated by
            # build_command_variants (LINK_VERBS has no "Update") -- these
            # are hand-added compact-spec commands, same pattern as Lineage
            # Linker's "Update Lineage Relationship".
            rel_guid = self._resolve_relationship_guid(object_type, attributes)
            if not rel_guid:
                raise ValueError(
                    f"Update {object_type} requires the relationship GUID. Provide it in `Certificate GUID`/`License GUID` (as returned by the original Link {object_type})."
                )
            if object_type == "Certification":
                properties = {
                ...
                body = body_slimmer({"class": "UpdateRelationshipRequestBody", "properties": properties, "mergeUpdate": True})
                await self.client._async_update_certification(rel_guid, body)
            else:
                properties = {
                ...
                body = body_slimmer({"class": "UpdateRelationshipRequestBody", "properties": properties, "mergeUpdate": True})
                await self.client._async_update_license(rel_guid, body)

            logger.success(f"Updated {object_type} relationship {rel_guid}")
