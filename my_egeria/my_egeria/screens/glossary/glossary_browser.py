...
class GlossaryBrowserScreen(BaseScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Vertical(
            ...
            Container(
                Horizontal(
                    Input(placeholder="Search...", id="search-input", disabled=True),
                    Button("Search", id="search-button", disabled=True),
                    id="search_row",
                ),
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected):
        # Capture selection for both modes, this will become capture tree node selected
